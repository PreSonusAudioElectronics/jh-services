...
/* Sysfs interface */
static ssize_t rpmsg_ivshmem_adapter_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct rpmsg_ivshmem_adapter_eptdev *eptdev = dev_get_drvdata(dev);
...
static ssize_t rpmsg_ivshmem_adapter_store(struct device *dev, struct device_attribute *attr,
...
static DEVICE_ATTR(console, S_IRUGO | S_IWUSR,
	rpmsg_ivshmem_adapter_show, rpmsg_ivshmem_adapter_store);
...
static int rpmsg_ivshmem_adapter_init_sysfs(struct rpmsg_device *rpdev)
{
...
static const struct file_operations rpmsg_ivshmem_adapter_eptdev_fops = {
	.owner = THIS_MODULE,
	.read = rpmsg_ivshmem_adapter_eptdev_read,
	.poll = rpmsg_ivshmem_adapter_eptdev_poll,
	.write = rpmsg_ivshmem_adapter_eptdev_write,
};
...
static int rpmsg_ivshmem_adapter_probe(struct rpmsg_device *rpdev)
{
	...
	eptdev->mdev.fops = &rpmsg_ivshmem_adapter_eptdev_fops;
	...
	ret = rpmsg_ivshmem_adapter_init_sysfs(eptdev->rpdev);
	if (ret)
		goto free_ept;
